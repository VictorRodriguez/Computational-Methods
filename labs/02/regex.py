...
def check_outside(x):
    parenthesis_num = 0
    something_outside_2 = False
    for i in regex:
        if (i == 'a' or i == 'b'):
            if(parenthesis_num != x):
                something_outside_2 = True
        if (i == '0' or i == '1'):
            if(parenthesis_num != x):
                something_outside_2 = True
        if i == '(':
...
for i in regex:
    ...
    if(i == '*'):
        if(last_parenthesis == True):
            ...
            if(something_outside == False):
                open_node = open_node
            else:
                if(check_outside(parenthesis_group_numbers[top-1]) == True):
            ...
            temp_2 = len(node_has_edge)
            if(curr_node < temp_2):
                node_has_edge[curr_node] = True
            else:
                node_has_edge.append(True)
            counter = 0
            for i in parenthesis_end_nodes:
        ...
        if(dont_check == False):
            if(regex[reg_counter + 1] == '*'):
                parenthesis_end_nodes.append(next_node)
                end_node_inside = search_open_paren()
                node_parenthesis.append(end_node_inside)
                parenthesis_end_nodes_history.append(curr_node)
